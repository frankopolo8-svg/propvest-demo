...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (isUiRequest(body)) return NextResponse.json({ ui: result.ui }, { headers: { "Cache-Control": "no-store" } });
...
function isUiRequest(value: unknown): value is UiRequest {
  const locale = value && typeof value === "object" ? (value as { locale?: unknown }).locale : undefined;
  return !!value
    && typeof value === "object"
    && (value as { type?: unknown }).type === "ui"
    && (locale === undefined || (typeof locale === "string" && /^[A-Za-z]{2,3}(?:-[A-Za-z0-9]{2,8})*$/.test(locale)));
}
...
function parseAssistantResponse(text: string) {
  ...
  try {
    ...
    if (typeof parsed.reply === "string" && parsed.reply.trim()) return { reply: parsed.reply.trim(), criteria: sanitizeCriteria(parsed.criteria), ui };
    if (Object.keys(ui).length) return { reply: undefined, criteria: {}, ui };
  } catch { /* handled below */ }
